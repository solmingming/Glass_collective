...
contract Execution {
    ...
    function isExecutable(uint256 proposalId) public view returns (bool) {
        (uint256 forVotes, uint256 againstVotes) = votingContract.getVotes(
            proposalId
        );
        uint256 totalVotes = forVotes + againstVotes;
        if (totalVotes == 0) return false;

        // 수정됨: 정밀도 손실을 방지하기 위해 곱셈을 먼저 수행
    ...
    function executeProposal(uint256 proposalId) external {
        require(
            ...
            "Execution: quorum not reached or proposal not executable"
        );
        Proposal.ProposalData memory p = proposalContract.getProposal(
            proposalId
        );
        require(
            p.status == Proposal.Status.Pending,
            "Execution: invalid status"
        );

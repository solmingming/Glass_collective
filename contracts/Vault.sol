...
contract Vault is AccessControl {
  ...
  constructor(address admin, address executor){ // address admin: 배포할때 전달받는 주소형 인자, 이 주소에게 관리자 권한 부여
    ...
    _grantRole(EXECUTOR_ROLE, executor); // 실행 담당자 역할 권한 부여
  }
  ...
  receive() external payable{
    // 의도적으로 emit 제외 빈 함수.. Ether만 받고 아무것도 수행 X
  ...
  // TREASURER_ROLE 권한을 가진 계정만 호출 가능 + 그렇지 않을시 해당 트랜잭션은 revert(철회?)됨
